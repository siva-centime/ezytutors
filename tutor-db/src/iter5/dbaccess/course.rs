...
pub async fn get_courses_for_tutor_db(pool: &PgPool, tutor_id: i32) -> Result<Vec<Course>, EzyTutorError> {
    ...
    let course_rows = sqlx::query!(
        "SELECT tutor_id, course_id, course_name, posted_time FROM ezy_course_c6 where tutor_id = $1",
        tutor_id
    )
    .fetch_all(pool)
    .await?;
...
pub async fn get_course_details_db(pool: &PgPool, tutor_id: i32, course_id: i32) -> Result<Course, EzyTutorError> {
    ...
    let course_row = sqlx::query!(
        "SELECT tutor_id, course_id, course_name, posted_time FROM ezy_course_c6 where tutor_id = $1 and course_id = $2",
        tutor_id, course_id
    )
    .fetch_one(pool)
    .await;
...
pub async fn post_new_course_db(pool: &PgPool, new_course: Course) -> Result<Course, EzyTutorError> {
    let course_row = sqlx::query!(
        "insert into ezy_course_c6 (course_id, tutor_id, course_name) values ($1, $2, $3) returning tutor_id, course_id, course_name, posted_time",
